...
{
	a_mem_obj = CECL_BUFFER(clGPUContext, CL_MEM_READ_WRITE, sizeof(DATA_TYPE) * N * M, NULL, &errcode);
	c_mem_obj = CECL_BUFFER(clGPUContext, CL_MEM_READ_WRITE, sizeof(DATA_TYPE) * N * M, NULL, &errcode);
	
	if(errcode != CL_SUCCESS) printf("Error in creating buffers\n");

	errcode = CECL_WRITE_BUFFER(clCommandQue, a_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * N * M, A, 0, NULL, NULL);
	errcode = CECL_WRITE_BUFFER(clCommandQue, c_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * N * M, C, 0, NULL, NULL);
...
void cl_load_prog()
{
	...
	clKernel1 = CECL_KERNEL(clProgram, "syrk_kernel", &errcode);
...
void cl_launch_kernel()
{
	...
	errcode |= CECL_SET_KERNEL_ARG(clKernel1, 1, sizeof(cl_mem), (void *)&c_mem_obj);
	errcode |= CECL_SET_KERNEL_ARG(clKernel1, 2, sizeof(DATA_TYPE), (void *)&alpha);
	errcode |= CECL_SET_KERNEL_ARG(clKernel1, 3, sizeof(DATA_TYPE), (void *)&beta);
...
int main(void) 
{
	...
	errcode = CECL_READ_BUFFER(clCommandQue, c_mem_obj, CL_TRUE, 0, M * N * sizeof(DATA_TYPE), C_outputFromGpu, 0, NULL, NULL);
