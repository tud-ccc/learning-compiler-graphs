...
void cl_mem_init(DATA_TYPE *A, DATA_TYPE *B, DATA_TYPE *x, DATA_TYPE *y, DATA_TYPE *tmp)
{
	...
	b_mem_obj = CECL_BUFFER(clGPUContext, CL_MEM_READ_WRITE, sizeof(DATA_TYPE) * N * N, NULL, &errcode);
	x_mem_obj = CECL_BUFFER(clGPUContext, CL_MEM_READ_WRITE, sizeof(DATA_TYPE) * N, NULL, &errcode);
	y_mem_obj = CECL_BUFFER(clGPUContext, CL_MEM_READ_WRITE, sizeof(DATA_TYPE) * N, NULL, &errcode);
	tmp_mem_obj = CECL_BUFFER(clGPUContext, CL_MEM_READ_WRITE, sizeof(DATA_TYPE) * N, NULL, &errcode);
	
	if(errcode != CL_SUCCESS) printf("Error in creating buffers\n");

	errcode = CECL_WRITE_BUFFER(clCommandQue, a_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * N * N, A, 0, NULL, NULL);
	errcode = CECL_WRITE_BUFFER(clCommandQue, b_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * N * N, B, 0, NULL, NULL);
	errcode = CECL_WRITE_BUFFER(clCommandQue, x_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * N, x, 0, NULL, NULL);
	errcode = CECL_WRITE_BUFFER(clCommandQue, y_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * N, y, 0, NULL, NULL);
	errcode = CECL_WRITE_BUFFER(clCommandQue, tmp_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * N, tmp, 0, NULL, NULL);
...
void cl_load_prog()
{
	...
	clKernel1 = CECL_KERNEL(clProgram, "gesummv_kernel", &errcode);
...
void cl_launch_kernel()
{
	...
	errcode |= CECL_SET_KERNEL_ARG(clKernel1, 2, sizeof(cl_mem), (void *)&x_mem_obj);
	errcode |= CECL_SET_KERNEL_ARG(clKernel1, 3, sizeof(cl_mem), (void *)&y_mem_obj);
	errcode |= CECL_SET_KERNEL_ARG(clKernel1, 4, sizeof(cl_mem), (void *)&tmp_mem_obj);
	errcode |= CECL_SET_KERNEL_ARG(clKernel1, 5, sizeof(DATA_TYPE), (void *)&ALPHA);
	errcode |= CECL_SET_KERNEL_ARG(clKernel1, 6, sizeof(DATA_TYPE), (void *)&BETA);
	errcode |= CECL_SET_KERNEL_ARG(clKernel1, 7, sizeof(int), (void *)&n);
...
int main(void) 
{
	...
	errcode = CECL_READ_BUFFER(clCommandQue, y_mem_obj, CL_TRUE, 0, N*sizeof(DATA_TYPE), y_outputFromGpu, 0, NULL, NULL);
